...
-- Create partial unique indexes instead of constraints (PostgreSQL syntax)
-- For wine-level slides (when package_wine_id is NOT NULL)
CREATE UNIQUE INDEX slides_wine_position_unique 
  ON slides (package_wine_id, position) 
  WHERE package_wine_id IS NOT NULL;

-- For package-level slides (when package_id is NOT NULL)
